  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4 flex items-center justify-between">
        Account Summary {isLoading && <span className="text-sm text-gray-500">Loading...</span>}
      </h2>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 p-3 rounded-md">
          ...
          <div className="text-sm mt-1">
            Unrealized P&L:
            <span
              className={`font-medium ml-1 ${(accountData.unrealizedPnL || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {(accountData.unrealizedPnL || 0) >= 0 ? '+' : ''}
        ...
        <div className="bg-gray-50 p-3 rounded-md">
          ...
          <div className="flex items-center">
            <span
              className={`text-xl font-bold ${(accountData.todayPnL || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {(accountData.todayPnL || 0) >= 0 ? '+' : ''}
              {accountData.todayPnL?.toFixed(2)}
            </span>
            <span
              className={`ml-2 flex items-center text-sm ${(accountData.todayPnLPercentage || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
